    ...
    pub fn try_match(&mut self, order: Order) -> Option<(Order, Order)> {
        match order.side {
            Side::Buy => {
                ...
                if let Some(sell_queue) = self.sells.get_mut(&order.price)
                    && let Some(matched_sell) = sell_queue.pop_front()
                {
                    // Clean up empty queue
                    if sell_queue.is_empty() {
                        self.sells.remove(&order.price);
                    }
            ...
            Side::Sell => {
                ...
                if let Some(buy_queue) = self.buys.get_mut(&order.price)
                    && let Some(matched_buy) = buy_queue.pop_front()
                {
                    // Clean up empty queue
                    if buy_queue.is_empty() {
                        self.buys.remove(&order.price);
                    }
